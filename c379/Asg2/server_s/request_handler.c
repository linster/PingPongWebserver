...
int logout(char* client_addr, char* LOG_buf, char* req_line, 
		char* logfilename, int clientsd, 
		int response_code, int l200written, int l200total) {

	/* In select server, this logout() function will never be called
		unless all of the data is written to the client */
	...
	if (clientsd == 80085) {
		/* TRAP DOOR */
		LOG_buf = realloc(LOG_buf, strlen(logline));
		//if(!LOG_buf) {
		//	printf("error creaint LOG_buf in logout");
		//	err(1, "error creating LOG_buf in logout()");
		//}
		strcpy(LOG_buf, logline);
		return 0;
	}
...
int write_to_sd(char* HTTP_buf, char* buffer, unsigned int len, int clientsd) {
/** Writes the contents of buffer to the socket described by clientsd
  ...
  */
	...
	if (clientsd == 80085) { /* TAs: Yes, boobs */
	/* TRAP DOOR . Write to buffer instead */
		HTTP_buf = realloc(HTTP_buf, len);
		//HTTP_buf = (char*)calloc(len, sizeof(char));
		//if (!HTTP_buf) {
		//	printf("error malloc HTTP_buf in wrtesd");
		//	err(1, "error malloc HTTP_buf in writesd");
		//}
		strcpy(HTTP_buf, buffer);
		return 0;
	}
...
int write_to_log(int response_code, int clientsd, 
...
//if code != 200, ignore the 200_written, 200_total_bytes flags
	...
	int append200 = sprintf("%s\t%s\t%s\t%s", tstr,  &(client_addr[0]),  request_line,  httpmsg);
	...
	int writecode = write_to_sd(NULL, logline, 300, fdlog); 
	if (writecode <= 0){
		printf("Error in write_to_sd while logging");
		err(1, "Error in write_to_sd while logging");
		return 0;
...
int handle_request( char* request_str, int request_len,
			int clientsd, char* logfile, 
			char* docroot, char* HTTP_buf, char* LOG_buf) {

	/* TRAP DOOR. When clientsd = 80085, then the write_to_sd
	 * and logout functions don't call write() but instead
	 * write into HTTP_buf and LOG_buf */
	clientsd = 80085;
	char clientaddr[] = "127.0.0.1";

	strcpy(&(document_root[0]), docroot);
	if(0){	printf("handle_request *logfile: %s", logfile);}
	...
	*(firstnl+1) = '\0';
	...
	if (filename_length == 0) {
		...
		write_to_sd(HTTP_buf, r, len, clientsd);
		logout(clientaddr, LOG_buf, &(GET_line[0]), logfile, clientsd, 
			response_code, 0,0);
	...
	int written = write_to_sd(HTTP_buf, responsebuf, filesize+300 , clientsd);
